...
"""
import os.path
from data_.base_dataset import BaseDataset, get_transform
from data_.image_folder import make_dataset
...
class ResizeNatural3Dataset(BaseDataset):
    ...
    def __init__(self):
        ...
        dataroot='datasets/reflection'
        BaseDataset.__init__(self)
        phase='train'
        if phase == 'train':
            self.natural_dir_A1 = os.path.join(dataroot, 'natural_' + 'T')
            self.natural_dir_B = os.path.join(dataroot, 'natural_' + 'I')

            self.natural_A1_paths = sorted(make_dataset(self.natural_dir_A1, float("inf")))  # load images from '/path/to/data/trainA1'
            self.natural_B_paths = sorted(make_dataset(self.natural_dir_B, float("inf")))  # load images from '/path/to/data/trainB'
            self.natural_size = len(self.natural_A1_paths)  # get the size of dataset A1

        self.crop = RandomCrop(256)
        self.dir_A1 = os.path.join(dataroot, phase + 'A1')
        self.dir_A2 = os.path.join(dataroot, phase + 'A2')
        self.dir_B = os.path.join(dataroot, phase + 'B')

        self.A1_paths = sorted(make_dataset(self.dir_A1, float("inf")))  # load images from '/path/to/data/trainA1'
        self.A2_paths = sorted(make_dataset(self.dir_A2, float("inf"))) # load images from '/path/to/data/trainA2'
    ...
    def __len__(self):
        ...
        length = max(self.A1_size, self.A2_size)
        # else:
